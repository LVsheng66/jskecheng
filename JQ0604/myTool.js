...
function setCookie(key,value,t) {
		...
		mydate2.setDate( mydate2.getDate()+t );
		console.log(typeof mydate2);
		console.log(typeof mydate2.toGMTString());
		//在存储cookie的时候 mydate2它是一个对象 为了防止拼接字符串的时候发生一些bug,一般建议 对日期时间对象转换为字符串形式 在拼接
		document.cookie = encodeURI( key+'='+value )+';expires=' + mydate2.toGMTString();
...
function getCookie(key) {
		...
		var arr = document.cookie.split("; ");  //['name=zhangsan','age=23'....]
		for(var i = 0; i < arr.length; i++){
			
			var arr2 = arr[i].split('=');  //['name', 'zhangsan']
			if(arr2[0]== key) {
