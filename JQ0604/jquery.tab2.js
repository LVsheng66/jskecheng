(function ($) {
	...
	function evFn(){
		...
		this.children('input').on(defaultSettings.evType,function () {
			...
			core.call(that,$(this).index());
		})

	}

	function core(n) {
		this.children('input').eq(n).addClass('active').siblings('input').removeClass('active');

		this.children('div').eq( n ).show().siblings('div').hide();
	}
	...
	function autoPlay(){
		...
		var timer = setInterval(function () {
			num++;
			if(num > defaultSettings.btnCon.length -1 ){
				num = 0;
			}
			core.call(that,num);
		},2000);
